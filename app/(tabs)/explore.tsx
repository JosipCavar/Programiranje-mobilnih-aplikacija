...
export default function ExploreScreen() {
  ...
  const [accelerometerData, setAccelerometerData] = useState<Vector3>({
    x: 0,
    y: 0,
    z: 0,
  });
  const [magnetometerData, setMagnetometerData] = useState<Vector3>({
    x: 0,
    y: 0,
    z: 0,
  });
  ...
  const getCurrentLocation = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Dozvola odbijena",
        "Lokacija se ne može dohvatiti bez dozvole.",
      );
      return;
    }
...
Točnost: ${Math.round(accuracy ?? 0)} m`,
  ...
  const takePhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Dozvola odbijena",
        "Kamera se ne može otvoriti bez dozvole.",
      );
      return;
    }
  ...
  const pickImage = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Dozvola odbijena",
        "Galerija se ne može otvoriti bez dozvole.",
      );
      return;
    }
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Primjeri rada s mogućnostima uređaja unutar React Native i Expo
          aplikacije.
        </Text>
      </View>
      ...
      >
        ...
        <ActionButton
          title={
            accelerometerEnabled
              ? "Zaustavi akcelerometar"
              : "Pokreni akcelerometar"
          }
      ...
      >
        ...
        <ActionButton
          title={
            magnetometerEnabled
              ? "Zaustavi magnetometar"
              : "Pokreni magnetometar"
          }
      ...
      >
        <Text style={styles.output}>
          Bluetooth: react-native-ble-plx NFC: react-native-nfc-manager
          Zvuk/mikrofon: expo-av ili noviji Expo audio paketi IoT: MQTT klijent
          i povezivanje s brokerom
        </Text>
      </HardwareFeatureCard>
    </ScrollView>
  );
